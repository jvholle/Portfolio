...
def initLogin():
    ...
    if request.form['action'] == 'Make a Comment':
        ...
        email = request.form['email']  # email is only for create user action.
        firstname = request.form['firstname']
        qryRow = queryDB(email, Users=True)  # id = iTupl[6]  **** Table are not the same*** claims don't match clients tables.
        #print("query row: "+str(qryRow))
        createUser = False
        # if user does not exist spec variables for obj.
        if not qryRow:
            createUser = True
            userObj = (firstname, None, None, None, email, None, None, date, None)  # None,
            print('Create a new partial user!')
        else:
            # instantiate the user object and pass to form as obj
            z = qryRow[0]
            userObj = models.User(z[0], z[1], z[2], z[3], z[4], z[5], z[6], z[7], z[8])  #, z[9])
            print(userObj)
        ...
        try:  # only insert into comments, if not email insert both user and comments.
            ...
            if form.errors:
                print(form.errors)
            return render_template('addcomm.html', form=form)  # name=mD[0][0])
            # if no existing user, create user, send to comment form.
            #elif qryRow is None:
        except (Exception, psycopg2.DatabaseError) as error:
            print(error)
    ...
    # create a seperate condition for each button with action.
        ...
        if form.validate_on_submit():  # request.method == 'POST' and form.validate():
            ...
            tuple(inForm)
            print("Update DB: " + str(tuple(inForm)))
            # create connection object to db  #conn = sqlite3.connect(sqlLoc)
            # Postgres DB conn
            conn = connPgres()
            c = conn.cursor()  # client_id, firstname, lastname, city, state, age, gender, xCoord, yCoord, date
            c.execute('''INSERT INTO comments (comment,date,user_id,given_name) VALUES(%s,%s,%s,%s)''', inForm)
...
def register():
    ...
    # Have not create a user. Below creates a user that was partially registered.?
    elif request.method == 'POST':
        # instantiate the user object and pass to form as obj
        U = models.User  # elif request.form['action'] == 'Register':
        userObj = U(None, None, None, None, None, None, None, None, None)
        # query emails from existing user table, if none, create user temp.
        # Change class object elements
        userObj.email = request.form['email']  # email is only for create user action.
        ...
        if form.validate_on_submit():  # request.method == 'POST' and form.validate():
            ...
            setattr(userObj, 'password', pw_hash)
            setattr(userObj, 'lastname', ln)
            insertTup = tuple([val for val in userObj.__dict__.values()])
            print(insertTup)
            # get current password from DB
            if qryRow:
                dbPW = qryRow[0][9]  # print(dbPW)
            # check db for existing users. if user does not exist spec variables for obj.
            if not qryRow:
                ...
                session['user'] = userObj.firstname
                session['email'] = userObj.email
                #return redirect('/')
            # partial registered user, get pw from DB, need add password and lastname
            elif userObj.email and not dbPW:  # user w email, no password
                # might need to do an update.
                addUserdb(insertTup)
                flash("Password was added: " + str(insertTup[1]))
                session['user'] = userObj.firstname
                session['email'] = userObj.email
                #return redirect('/comment')
            # there is an email and pw. Check pw. in the DB.
        ...
        userObj.firstname = form.firstname.data
        userObj.email = form.email.data
        '''if form == forms.AddComment:
...
def editComm():
    if request.method == 'POST':
        ...
        # initiate login, otherwise, cannot edit comments.
