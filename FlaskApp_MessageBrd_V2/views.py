    ...
    if request.form['action'] == 'Make a Comment':
        ...
        try:  # only insert into comments, if not email insert both user and comments.
            ...
            if form.validate_on_submit():  # request.method == 'POST' and form.validate():
                ...
                flash('User inserted successfully!')
                return redirect('/')
            # GET Define form.fld.data vals from sel. row to pass to form.
            # if existing user
            if qryRow:
                user_id = qryRow[0][0]
            ...
            # create partial new user before comment form opened
                ...
                try:
                    ...
                    c.execute(  # if id is auto generated, used RETURNING id to gen. the id after insert.
                        '''INSERT INTO users (firstname,lastname,city,state,email,xcoord,ycoord,date,password) 
                            VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING user_id;''', insertTup)  # Error, object not support subscripting
                    user_id = c.fetchone()[0]
                    conn.commit()
                    flash('User inserted successfully!')
                    c.close()
...
def register():
    """ Register/Login users. In work: Two register options: Soft and full """
    date = datetime.datetime.today()
    ...
    # Soft and full Register/Login of users.
        ...
        if form.validate_on_submit():  # request.method == 'POST' and form.validate():
            ...
            if not qryRow:
                ...
                addUserdb(insertTup)
                print("user was added: " + str(insertTup[0] +" "+ str(insertTup[1])))
            ...
            # there is an email and pw. Check pw. If incorrect pw, redirect to index for login.
                ...
                if ck_hash:  # flash("You are already registered, please login.")
                    ...
                    # set cookie for user, if they don't have one. Check email from db match email from form.
                    #if 'email' in session and session['email'] is not userObj.email:
                    session['user'], session['email'] = userObj.firstname, userObj.email
                    return redirect('/')
                else:
...
def delete(id, user=False):
    try:
        ...
        c = conn.cursor()  # client_id, firstname, lastname, city, state, age, gender, xCoord, yCoord, date
        comm_id = (id,)
        c.execute('''DELETE FROM comments WHERE comm_id=%s''', comm_id)  # user_id,
        conn.commit()
